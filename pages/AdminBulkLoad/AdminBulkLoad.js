...
async function submitBulk() {
    ...
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            currentCommandsBytes + additionBytes + 1024;

            const wouldExceedBytes = currentLines.length > 0 && estimatedJsonBytes > MAX_REQUEST_BYTES;
        ...
        setLoaderProgress("Prepared bulk batches:", batches.map(b => ({
            startLine: b.startLine,
            approxBytes: encoder.encode(JSON.stringify({
        ...
        for (const batch of batches) {
            ...
            try {
                ...
                console.error("Bulk error for batch starting at line", batch.startLine, err);
